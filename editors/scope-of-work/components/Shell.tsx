...
export function Shell({
  ...
  // The vault can ask for a specific project/section when it opens this
  // document (see shared/sow-intent.ts). Read in the initialiser so the first
  // paint is already the requested view — a useEffect would flash the
  // overview first. The read is one-shot and id-checked.
  const [view, setView] = useState<View>(
    () => readSowIntent(document.header.id) ?? { kind: "overview" },
  );
