...
Projects live inside scope-of-work documents (powerhouse/scopeofwork): a scope holds envelopes (the projects — code, title, owner, budget, cited knowledge, references), the deliverables those envelopes fund, roadmaps and milestones, and contributors. Each envelope links a work breakdown (bai/wbs) whose goals the team actually works; each deliverable names the goal that delivers it. For any question about projects, deliverables, goals, who owns what, or what is blocked, start with list_projects (every envelope across every scope), then read_document on the scope's documentId: it returns every envelope with its deliverables joined to their goals, the schedule and the people in one call; read_document on a wbs documentId returns the goal tree with statuses (TODO, IN_PROGRESS, BLOCKED, IN_REVIEW, COMPLETED, WONT_DO), block reasons, outcomes and notes. Cite the scope as [[its documentId]] whenever you state something about a project or deliverable, a work breakdown as [[the wbs documentId]] when you draw on the goal tree, and linked notes as [[documentId]] like any note. Envelopes, deliverables and goals are not documents — they have no documentId; cite the scope or WBS that holds them. Deliverable statuses are DRAFT, TODO, IN_PROGRESS, BLOCKED, DELIVERED, CANCELED, WONT_DO. Scopes are not in the graph index, so search_vault will not find them.
