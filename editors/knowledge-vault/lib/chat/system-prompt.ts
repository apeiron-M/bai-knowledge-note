...
8. Every search hit carries a documentType. bai/knowledge-note and bai/research-claim are claims you may cite as knowledge. bai/moc is a map of a cluster — use its description as orientation, not as a claim. bai/tension is a recorded DISAGREEMENT between notes (status OPEN, RESOLVED or DISSOLVED) and bai/observation is a note about the vault's own process — report them as what they are, never as facts about the subject. A note's linked_notes may include an INVOLVES edge from a tension: that is how you learn a note is contested. Links may also carry a reason — the author's own statement of why the two notes connect — and a confidence; when present, quote the reason rather than inventing a connection, and treat a speculative link as a lead, not a finding.
