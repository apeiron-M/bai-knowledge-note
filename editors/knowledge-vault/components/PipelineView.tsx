...
export function PipelineView() {
  ...
  const state = useMemo(() => {
    ...
    return (queueDoc.state as unknown as { global: Record<string, unknown> })
      .global;
  }, [queueDoc]);
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          <div
            key={s.label}
            className="rounded-lg bg-[#1e1e2e] p-3 ring-1 ring-white/5"
          >
      ...
      {tasks.length === 0 ? (
        <div className="rounded-lg border border-dashed border-white/10 p-6 text-center">
          ...
          <p className="mt-1 text-xs text-gray-600">
            Use /seed to ingest source material and start processing
          </p>
        </div>
      ) : (
        <div className="space-y-1">
          {tasks.map((task) => (
            <div
              key={task.id}
              className="flex items-center gap-3 rounded-lg border border-white/5 bg-[#1e1e2e] px-4 py-3"
            >
              {/* Status indicator */}
              <span
                className={`text-xs font-semibold ${STATUS_COLORS[task.status] ?? "text-gray-500"}`}
              >
                {task.status}
              </span>
              ...
              <div className="flex items-center gap-1">
                {task.completedPhases.map((phase) => (
                  <span
                    key={phase}
                    className={`h-2 w-6 rounded-full ${PHASE_COLORS[phase] ?? "bg-gray-500"} opacity-60`}
                    title={`${phase} (done)`}
                  />
                ))}
                {task.currentPhase && (
              ...
              {/* Type badge */}
              <span className="rounded bg-[#313244] px-1.5 py-0.5 text-[10px] text-gray-500">
                {task.taskType}
              </span>
