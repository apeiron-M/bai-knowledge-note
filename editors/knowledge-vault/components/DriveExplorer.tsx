...
type ViewMode = "notes" | "graph" | "sources" | "pipeline" | "health" | "config";
...
export function DriveExplorer({ children }: EditorProps) {
  ...
  const notesFingerprint = useMemo(() => {
    return notes.map((n) => `${n.id}:${n.title}:${n.status}:${n.links.length}:${n.links.map((l) => l.targetDocumentId).join(",")}`).join("|");
  }, [notes]);
  ...
  const pipelineDocId = allFiles.find((n) => n.documentType === "bai/pipeline-queue")?.id;
  const vaultConfigDocId = allFiles.find((n) => n.documentType === "bai/vault-config")?.id;
  const graphDocId2 = allFiles.find((n) => n.documentType === "bai/knowledge-graph")?.id;
  ...
  const TABS: { key: ViewMode; label: string; badge?: number; icon: React.ReactNode }[] = [
    ...
    {
      key: "config" as ViewMode, label: "Config",
...
function CreateMenu() {
  ...
  const [dialogItem, setDialogItem] = useState<{ label: string; type: string } | null>(null);
          ...
          <div className="absolute right-0 z-20 mt-1 w-48 rounded-lg border border-white/10 bg-[#181825] py-1 shadow-xl">
            {CREATE_ITEMS.map((item) => (
              <button
                key={item.type}
                type="button"
                onClick={() => { setDialogItem(item); setMenuOpen(false); }}
