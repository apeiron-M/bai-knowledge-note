...
**Scope of this spec:** the _conversion capability_ — an out-of-process document-conversion service plus a `subgraphs/convert` that proxies it and turns its output into a source-shaped **plan**. It creates no documents and ships no UI.
...
- **Format loss.** Tables, headings, footnotes and page structure are what a PDF _is_; `textContent` is what survives a copy-paste. A note extracted from a flattened table cites a source whose table no longer exists.
- **One paste, one document.** A 300-page book becomes a single `bai/source`, and `AGENT.md` is explicit about the cost: _"every operation stores a full JSON copy of the document's state, so each `ADD_EXTRACTED_CLAIM` on a book-sized source re-serialises the whole book."_
...
- The split must follow the document's own authored structure, and the user must see it _before_ anything is written.
- The agent path (`powerhouse-knowledge` plugin, `switchboard` CLI, REST) and the app path should reach the same implementation.

**Non-goals for this spec:** creating documents, queueing tasks, the Intake UI, OCR-quality tuning, transcription of audio/video (the format is _accepted_ by the backend; nothing in this plan exercises it).
...
**The `{markdown, chunks}` contract is a requirement on the backend, not a fact about it.** docling-serve answers a _different_ shape — `{ document: { md_content, json_content, html_content, … }, status, processing_time, timings, errors }` — so it cannot be pointed at directly. Any backend is expected to be fronted by a small adapter that speaks this contract: the local Bun service _is_ that adapter for `docling.rs`, and a ~40-line Bun adapter would do the same for `docling-serve` (map `document.md_content`, then issue the chunking request with `chunking_options`). That keeps the vault package backend-agnostic — one client, one shape — and it means adopting docling-serve later costs an adapter, not a change to `subgraphs/convert`.
...
POST /convert?filename=<url-encoded>[&ocr=1]
  ...
  → 200 { markdown, chunks: [{ text, headings?, docItems?, contextualized? }],
          ...
          normalised: null | "qpdf" | "gs", ocr: null | "forced" | "requested" }
...
- **Chunk with `chunkFile()` — the cheaper JSON path was tried and rejected on fidelity.** The binding's typing invites the shortcut: _"a document converted once … chunks without re-converting"_, via `chunkDocument(jsonContent)`, and it is genuinely faster — on the same 2-page CV, JSON-chunking measured **890 + 885 ms** against `chunkFile`'s **1 480 + 1 510 ms**, ~1.6×. But the fidelity check the plan made mandatory **failed on real content**: the two paths agree on chunk _count_ (55 each) and on every heading path, and then diverge at character 4 145 of the normalised text, inside the CV's education table —
...
- **How a PDF gets read is decided from the file and the machine (2026-09-18) — the OCR story, third and final version.** The first fix routed a garbled text layer to docling's own OCR; the second found that only the module-level call honours `forceFullPageOcr`; this one replaces both with a ladder, because two measurements changed the problem. (1) **The Sky report's text layer was never broken — pdfium was.** pdf.js, pure npm, read it at 1.2 % single-character tokens in 0.4 s for 19 pages where pdfium gave 96 %; poppler agreed (1.2 %, 0.3 s). So the whole class "re-exported PDF with Type 3 fonts" needs no OCR at all, on any machine. (2) **Tesseract is 3–8× cheaper than docling's ONNX OCR for equal quality** (2.6 s/page single-threaded, parallel across cores, vs 8.6 s/page), and the Ghostscript rewrite does nothing for fonts (0.964 after rewriting).
  A correction the live test forced: **docling's plain pass already OCRs image-only pages** — a 2-page synthetic scan came back at rung 0, 1.8 % garble, 4.2 s/page. So the ladder's OCR rungs are for what that pass gets _wrong_, not for scans as such. The ladder, in `scripts/docling-serve/routing.mjs` (pure, tested): **0** docling plain pass (reads born-digital PDFs and plain scans) → **1** pdf.js text when only pdfium is garbled (text, no layout; sections by size) → **2** Tesseract via `ocrmypdf` (`--skip-text` for a scan, `--force-ocr` for a layer that lies to pdf.js too; docling then reads the result normally, so headings and tables survive) → **3** docling's bundled OCR, automatic under a time budget (`CONVERT_AUTO_OCR_SECONDS`, 60) and **offered** above it (`needsOcr` + estimate; `?ocr=1` accepts) → **`415` with `capabilities`** when nothing can. Capabilities are **detected at first need and reported in `/health`**, never installed by the package: `tesseract`/`ocrmypdf`/`qpdf`/`gs`/`nice` on PATH, the docling models, `DOCLING_RS_EP`. A server deployment bakes every rung into the service's own glibc container (`scripts/docling-serve/Dockerfile`); a laptop with nothing installed still has rungs 0, 1 and 3; the landing tells the user which of _fast / slow / unavailable_ scans are here.
  Two guards regardless of engine, both from the live test in which a 163 s OCR pass pinned a 16-core laptop: **one conversion at a time** (`503 CONVERSION_BUSY` to a second caller, which the client treats as retry-later, not outage), and **`nice -n 10`** on the autostarted service and on the OCR helper. The garble detector keeps its 0.5 threshold on purpose: a correctly read maths paper sits at 0.30 (variables, `F 2`), and it only runs when docling's own pass is garbled. `textSource` and `ocr` in the response say which rung produced the text, so the review can tell the reader what to distrust.
...
- **The real efficiency lever is the warm pipeline.** The package exports a `Pipeline` class that _"keeps the ONNX models loaded across calls"_. A long-lived service should hold one, rather than calling the module-level functions per request and paying model load each time. Measured on the second and later requests, this is the difference between per-request constant overhead and actual conversion cost.
- `ConvertOptions` exposes a **PDF page window** (`pages: "A-B"`, or `"N"`, 1-based inclusive) — the only way to convert _part_ of a book without rewriting the file, and therefore the tool for a fast fidelity check on a 238-page document.
- **A PDF that pdfium refuses needs a normalising retry — now verified, though the original trigger stopped reproducing.** During the spike a valid, unencrypted, 238-page O'Reilly PDF failed at load with `pdfium error: PdfiumLibraryInternalError(FormatError)` while `pdfinfo` read it happily, and converting a `gs -sDEVICE=pdfwrite` rewrite of the same pages succeeded. So the service treats a rewrite as a retry path, not a default (`convert → refusal → rewrite with qpdf, then gs → convert after each → still failing ⇒ 415`), because rewriting every PDF up front costs time for no benefit on the files that already work.
  ...
  **The fixtures immediately found a bug in the first implementation**, which is the argument for having built them rather than moving on. That version tried `qpdf`, treated **exit code 0 as proof of success**, and handed the still-broken file back to the converter:
  ...
  So `count_mismatch.pdf` answered **`500` with a raw pdfium error** while `gs` sat right there able to cure it. The repair loop now **converts after each rewrite and takes the first that works**, and answers **`415`, not `500`**, when none does — the rule being _the tool exiting 0 is not the test; the rewritten file converting is_. A second correction from the same exercise: the trigger is not a literal `FormatError` match. Both fixtures throw `PdfiumLibraryInternalError(Unknown)`, so the check matches the whole `PdfiumLibraryInternalError` family.

  One honest limit: with `gs` in the loop, a file it can recover will now succeed where a `415` might have been clearer. A 214-byte random "PDF" came back `200 normalised=gs` with empty markdown. Whether the _route_ should treat an empty result as a failure is open — but it cannot be decided in the service, because an image-only PDF scanned without OCR legitimately yields little text (§13).

#### Progress and the extraction score (2026-09-18)

Two additions to the service's contract, both **measured, never estimated**:

- **`?job=<id>` on `POST /convert` and `GET /progress/:id`.** The caller picks the id (a UUID) and polls `GET /progress/:id` → `{ phase, pages, pagesDone, elapsedMs }`. `pages` comes from pdf.js, read once up front for every PDF (0.4 s for 19 pages); `pagesDone` comes from the warm pipeline's `streamFileMarkdown`, which yields one chunk per finished page — measured on the 23-page paper: 24 chunks, first at 2.4 s, last at 10.1 s, **byte-identical to the buffered `convertFileAsync` result**, so switching the plain pass to the stream costs nothing. Phases: `starting → reading (bar) → structuring (docling's chunking pass, no page signal: a phase, not a bar) → done`, with `text-layer` / `ocr` when the routing ladder takes a lower rung, and `failed` on any refusal. Jobs linger a minute after completion, then `404 JOB_NOT_FOUND` — the poller's cue to stop. Live trace on the paper: `1s reading 0/23 … 7s reading 18/23, 8s structuring 23/23 … 16s done`.
- **Rung 1 is no longer flat (2026-09-18).** The Sky report came through pdf.js as 811 lines with no heading and no paragraph — unreadable as a source. But the text layer carries geometry, and it is regular: body text at one dominant size (20 pt, 3 008 chars), section headings at 1.5× (30), page titles at 3× (60), lines 1.3 sizes apart inside a paragraph and 2.6 apart between them. `textlayer.mjs` (pure, 8 tests) turns that into markdown: the modal size by characters is the body; ≥ 1.35× is `##`, ≥ 2.4× is `#`; a gap over 1.7 sizes, a size-class change, a jump back up the page (new column) or a `\n\n` inside a run starts a new block; runs on one line are glued without a space when the x-gap is under 0.15 sizes (pdf.js splits at font changes). Measured on the report: **29 headings, 370 blocks, 425 ms**; the paper's coverage score is unaffected because `text` (flat) is kept for the garble check and coverage while `markdown` (structured) becomes the document. Honest limits: tables come out as lines, bullets are not inferred (indent is unreliable), 15 pt-over-14 pt card labels are bold rather than bigger and stay in their paragraph, and spaces the PDF put inside words ("und erlying") are in the file itself. For tables the response now carries **`ocrOffer: { via, estimateSeconds } | null`** whenever `textSource` is `pdfjs` — the same `decideRoute` with `forceOcr: true` — and the review pane offers "Re-read with OCR" (Tesseract gives docling a clean layer, so tables return). **Measured, the forced re-read of this report (fixed rule, `--force-ocr`, 8 cores): 156 s** — ocrMs 113 s, convert 20 s, chunk 23 s, i.e. **8.2 s/page**, where the offer had said "~5 s" (the old 2.6 s/page ÷ cores figure came from a `--skip-text` run that OCR'd nothing). `TESSERACT_SECONDS_PER_PAGE` is now 8 for the whole re-read, not divided by cores, so a 19-page file is offered (152 s > the 60 s budget) rather than run unasked. Quality of what comes back: readable (single-letter ratio 0.09), 37 headings, **54 table rows**, coverage 88 % — but **OCR misreads digits**: the Key Metrics header row read `25 | 275 | 375 | aazs | aims` for Q1 2025 … Q1 2026, `7]` for 7.1, `101` for 10.1. The offer says so ("numbers will need checking"); for a financial report the text-layer rung (100 %, correct digits, no tables) is the better source and OCR is for tables a reader will verify. Two guards from the same run: OCR output that still reads as soup falls back to the structured text layer instead of being returned (the user's first click, before the mode fix, produced 0.9 % coverage — the score caught it), and OCR's doubled spaces between words are collapsed in prose (tables and code untouched). **A rule the first click exposed:** the Tesseract mode was `--force-ocr` only when pdf.js was garbled too, else `--skip-text` — which OCRs only pages _without_ text. Every page of this report has (undecodable) text, so a forced re-read would have kept every layer and handed docling the same soup. Now: docling's read garbled → `--force-ocr` (the layer is what defeats it and must go, whatever pdf.js thinks); only a forced OCR of a file docling read fine keeps `--skip-text` (OCR the image pages, keep the trustworthy text).
- **Figures (2026-09-18, `?figures=1`; plan `docs/superpowers/plans/2026-09-18-source-figures.md`).** Decoding formulas was measured and rejected (CodeFormulaV2: 79 formulas not finished in 10 min on CPU, > 7.5 s each). Instead the document's pictures and display formulas come back as PNGs. docling's JSON carries every picture's box (and a 72 dpi embed) and every formula as a text item with `prov: []`; `figures.mjs` (pure, 13 tests) locates a formula by the vertical gap between its positioned neighbours in reading order and tightens the crop to its interior ink band (the neighbours' boxes include their line spacing). Pages are rendered once with Ghostscript at 200 dpi (156 ms a page) and cropped with `sharp` (13 ms); pictures are cropped from the same render (2.8× sharper than the embed), falling back to the embed without `gs`/`sharp`. **Measured on the paper: 3 pictures + 70 of 79 formulas (7 cross a page, 2 have their next item above their previous), 285 KB of PNG, 25.7 s** (JSON pass 6.6 s, renders + crops 4.5 s).

  **Resolution is per figure, from its width (2026-09-18).** One setting cannot serve both: at 72 dpi the Sky charts are sharp (~1300 px) but a display formula is ~240x34 and its superscripts are mush, and at 200 dpi the Sky report spends 24.7 s rendering pages. So each figure asks for about `CONVERT_FIGURE_TARGET_PX` (1000) of width, bounded by `CONVERT_FIGURE_MIN_DPI` (72) and a cap that depends on what the crop is for — 200 dpi for a formula, read glyph by glyph; 100 for a picture, read whole. docling's own embed is used instead of a render when it is at least as good (or already ≥ 600 px), and anything wider than the target is downscaled with sharp, because a 2655 px chart is 2 MB in the attachment store for no gain. **Measured end to end: Sky 41.8 s / 25 charts at 1000x564 / 1.2 MB with renders reduced to 114 ms; paper 25.7 s / 73 figures / 285 KB with formulas at 976x210.** Only when docling wrote the markdown (`textSource ≠ pdfjs`): the text-layer rung has no placeholders. Each figure is keyed to the n-th `<!-- image -->` / `<!-- formula-not-decoded -->` (`placeholderIndex`) and carries alt text a model can read as text (`formula 12, page 7 — not decoded`). Budget `CONVERT_FIGURES_MAX_BYTES` (8 MB): formulas dropped first. `/health.capabilities` gains `sharp`. The client side is done the same day: `bai/source` gained a **general** `attachments: [SourceAttachment!]!` (`role` is free text — "picture", "formula", tomorrow "table" or "clip" — because the reference read model is schema-driven and only `AttachmentRef`-typed fields make a ref _claimed_), with `ADD_ATTACHMENT` / `REMOVE_ATTACHMENT`; the intake hashes every figure with the original, rewrites each source's slice to `![alt](attachment://…)` (indices made local with `Section.placeholderBase`), claims them in the same `POST actions` batch as the original, uploads once; the shared markdown preview resolves `attachment://` images through the attachment service. A source with no attachments reads `[]`.

- **Figures on the text-layer rung (2026-09-18).** A file whose fonts defeat docling's _text_ still has its pictures found by docling's _layout_ model, which reads pixels — but the pdf.js markdown that ships for such a file has no `<!-- image -->` placeholders to put them in, so the Sky report came out with 0 figures while docling had located 25. Both sides speak the same page coordinates, so `textlayer.mjs` now keeps each block's vertical span (`documentToBlocks`) and `insertFigurePlaceholders` puts a figure before the first block that starts below its top, page by page, appending those below all text and reporting a box-less figure as `unplaced`. The placeholders are then renumbered in the order they appear and the markdown is re-chunked, because sections are cut from it. **Measured on the 19-page Sky report: 25 charts, 905 KB, all 25 placed, `textSource: "pdfjs"`, 76.5 s total** — JSON pass 14.7 s, page renders 24.7 s, the rest the conversion itself (up from ~30 s without figures). `unplaced` is counted separately from `droppedForBudget`: not being positionable and being left out for size are different failures.
- **Tables on the text-layer rung (2026-09-18).** docling writes tables into its own markdown, but only where its layout model calls something a table — and on the Sky report it called **nothing** a table (25 pictures, 0 tables), so the appendix reached the source as rubble: `"ACCOUNT"`, `"$96.01M 100%"`, `"—"`, a fragment per block. The numbers were never lost; pdf.js reads them exactly. `tables.mjs` (pure, 13 tests) recovers the shape from the positions: runs sharing a baseline are a row, the whitespace that persists down the page separates columns, and a band of rows filling several columns with short value-shaped cells is a table. Two findings from the real page are load-bearing: **columns come from whitespace, not left edges** (money is right-aligned — `$96.01M` at x=799 and `100%` at x=843 are one column; clustering starts gave 15 columns where there are 6), and **a table's own label rows are not tabular** ("Stability Fee Revenues" alone on its line cut one table into three until short non-tabular runs were allowed inside a band). The header row is pulled in from just above, because without `ACCOUNT | Q1 ‘25 | …` the figures mean nothing. Two-column tables are accepted on stricter evidence (≥ 5 rows, ≥ 85 % values on the data rows) since two columns are also what prose with a sidebar looks like. **Measured on the Sky report: 3 tables — quarterly P&L 24x6, monthly 24x4, balance sheet 16x2 — 67 markdown rows, digits intact (`$13.03B`, `$123.79M`), 46.9 s.** What is deliberately _not_ done: an account name that sits between its value row and its percentage row, equidistant from both, is left on its own row rather than merged into either — a mis-attributed financial figure is worse than a verbose table. Prose and the report's four-column card grid stay prose (the value test), and the detector runs only on the text-layer rung, because where docling detects a table its own markdown is already right.
- **A figure's own text (2026-09-18).** A chart draws its labels as live text on top of its picture, so they are in the text layer _and_ in the image. Left alone they reach the source twice: once in a picture the reader can see, once as rubble underneath — on the Sky report's collateral chart, `"$14B"`, `"$12B"`, `"$615.44M"` and 35 more, **38 of that page's 93 runs, 109 of the document's 1117 (9.8 %)**. `absorbFigureText` gives them to the figure: the largest line becomes its caption — and from there the alt text a model reads before fetching the image (`figure 11, page 7 — Protocol Collateral Composition, Q1 2025 – Q1 2026`) — and the rest becomes one `*Figure text: …*` line. Nothing is discarded, so coverage stays 1.0. Only lines under `FIGURE_LABEL_MAX_CHARS` (120) are taken: a picture that overlaps a paragraph — a page background, a scan — must not swallow the document's prose, and length is the difference that holds. Measured: 4 charts captioned, prose intact.
- **Tables where the page has two things side by side (2026-09-18).** Rows are grouped by baseline across the full page width, so a chart standing beside a table lands in the table's rows and a chart beside prose makes prose look tabular. Three structural rules, each a test: a row qualifies on **its own value columns** (not on everything sharing its baseline); columns are read from **value-carrying rows only** (the heading "Sky Protocol Key Metrics, Q1 2025 – Q1 2026" spans x 75..639 — every column of the table beneath it — and projecting that span merged all six into one, which is why that table was invisible); and a **header is adopted only when its cells land inside the columns already established**, so a section heading spans them rather than becoming a row. Two guards stop the opposite failure: an **edge column whose top cell is a number and which stands further off than the table's own column spacing** is another block of content (page 8's chart ticks), and a **two-column table needs a key on every data row** — without it, page 5's paragraph fragments beside a chart's axis read as a 15-row table and the prose would have been _replaced_ by it. **Measured across 7 PDFs / 268 pages** (report, invoice, CV, profile, a 238-page book, a maths paper): Sky 4 tables — metrics 5x6, quarterly P&L 24x6, monthly 24x4, balance sheet 16x2 — and **0 tables, 0 false positives everywhere else**. The invoice's own table is missed: one data row, below `minRows`. That bias is deliberate — the detector runs only where docling's text is garbled, so a miss costs a table while a false positive costs the page's prose.
- **`quality` in the 200 body**: `{ coverage, rawTokens, formulas: { total, decoded }, images }` from `quality.mjs`. `coverage` is the share of the pdf.js text layer's tokens (≥ 2 chars, lower-cased) that appear in the markdown — a **floor** on completeness, not a proof (a token can be present in the wrong place); `null` when the layer has < 40 tokens or the file is not a PDF. `formulas` and `images` are the losses docling itself announces (`<!-- formula-not-decoded -->`, `<!-- image -->`). Measured: Sky report 100 %; a CV 98.2 %; the maths paper **96.9 %, 79 formulas undecoded, 3 figures** — the 3.1 % gap is those formulas' glyphs. Not persisted on the source (no schema change); shown in the review header and on the file row.

### 5.2 The subgraph route

```
GET  /api/@powerhousedao/knowledge-note/convert/health          auth: renown
GET  /api/@powerhousedao/knowledge-note/convert/progress/:job   auth: renown   → 200 { phase, pages, pagesDone, elapsedMs } | 404 JOB_NOT_FOUND
POST /api/@powerhousedao/knowledge-note/convert                 auth: renown
     ?filename=<url-encoded>&markdown=1&minSectionChars=<int>&ocr=1&job=<id>
     body: raw octets
  → 200 {
      ...
      quality: { coverage, rawTokens, formulas: { total, decoded }, images } | null,   // measured completeness, see above
      timings
    }
  → 400 FILENAME_REQUIRED | EMPTY_BODY | INVALID_MIN_SECTION_CHARS
  (the 200 body also carries `ocr: null | "forced" | "requested"`, passed through from the service)
...
- **Registration order:** routes match in registration order across the package's single HTTP namespace, so `convert/health` is registered before nothing conflicting and there is no `convert/:id`; the paths are `convert`, `convert/health` and `convert/progress/:job`; the parameterised one is registered last and cannot shadow the other two.
...
```json
{
  ...
  "headings": ["The Vault"],
  "docItems": ["#/texts/1"],
  "contextualized": "The Vault\nIntro paragraph."
}
```

A 400-page book scales that to thousands. Sources at that granularity would recreate inside the vault exactly the dilution `AGENT.md` warns about — _"noise never announces itself, it just dilutes every query that touches its topic forever."_
...
| ----------------- | ------------- | ------------ | --------- | --------------- |
| 2 000 _(default)_ | **107**       | 10           | 2         | 12 770          |
| 3 000             | 80            | 7            | 1         | 13 473          |
| 4 000             | 62            | 5            | 1         | 12 770          |
| 6 000             | 45            | 4            | 1         | 17 385          |
| 8 000             | 35            | 3            | 1         | 17 385          |
...
Two real documents were converted during the spike, and both damage the assumption that a heading path is trustworthy. Recorded here rather than in §11 because they are arguments about _the rule_, not about the pipeline.

**A 20-page PDF sample of a published book** (238 pages total, O'Reilly trim size; see §11 for the numbers) came back with **97 of 99 chunks at heading depth 1 and 2 at depth 0** — the hierarchy is _flat_. Every chapter heading, every part title, and every piece of front matter is level 1, so there is no depth at which to nest. Worse, the level-1 set includes furniture: `[ contents ]`, `[ SIDE NOTE ]`, `Warning`, `Conventions Used in This Book`, `Praise for …`, and two spellings of `PART III …` differing by a colon. Cutting at depth 1 would mint a section per heading, several of them noise.
...
**A two-page two-column CV** returned 54 chunks and 5 552 characters, and its heading paths are actively wrong: chunks under the employer `Powerhouse` carry `headings: ["Remote"]`, and chunks under two later employers carry `["Copenhagen, Denmark"]` — the _location_ column was read as a heading. Cutting at depth 1 would produce sections named after cities.
...
1. **The heading path is a hint, not an authority.** `deriveSections` must not treat its output as ground truth, which is precisely why the spec insists the section list is _visible and editable before anything is created_ (Task 3's preview-only route, §12). That requirement is now load-bearing rather than a nicety.
2. **The nesting question is a backend question.** The Rust binding's `ConvertOptions` has **no heading-hierarchy inference** — verified against its typings; there is no `do_pdf_heading_hierarchy` equivalent, while docling-serve exposes one that infers levels from bookmarks, outline numbering _and_ font style. Flat output is therefore expected from the local service and is a reason to keep docling-serve as a second backend rather than a fallback.
...
Model storage (`DOCLING_RS_HOME`) is a property of the _service_, not the vault, and stays out of this package's config.
...
| docling failed on the bytes         | `500 CONVERT_FAILED`, message passed through | A scan without models is the common case: the service reports `missing: ["pdfium","layout_heron.onnx"]` and the route surfaces it, so the UI can say _why_ rather than "failed". |
...
```json
{
  "error": "parse error: pdf: pdfium error: PdfiumLibraryInternalError(FormatError)",
...
**The extrapolation was wrong in an instructive direction.** Paging the 20-page sample (~2.3 s/page) across 238 pages predicted ~9 minutes; the real figure is **3 m 33 s, about 0.9 s per page**. The difference is content, not measurement: those twenty pages are front matter — praise, contents, dense multi-column layout, images — while the body is single-column prose. Per-page cost therefore varies by roughly 2.5× with page complexity, so any progress estimate the UI shows must be derived from _pages converted so far on this document_, never from a constant.
...
- **Chapter-sized is the right _size_, even though detection is unreliable.** 367 840 characters across a book with roughly fifteen chapters averages ~24 500 characters per chapter — comfortably under the ceiling, and about the size a human would want to open and read. That validates the granularity in §6 while leaving its _detection_ as the open problem: 289 heading paths is not fifteen chapters.
...
Heading fidelity, from the samples: tables survive as markdown tables (`main_example.pdf`'s Education block, the PO's line items) — but see §6.2 for what the _heading paths_ did on both, which is the more consequential finding.
...
**Level inference is real, and it is not clean.** `docling-serve` genuinely nests: `[ Preface ]` at L1 with `Why I Wrote This Book` at L4 beneath it is _correct_, and `[ contents ]` and `PART III …` land sensibly at L1. But **15 of its 27 headers sit at level 6** — a grab bag containing cover blurbs (`LAURA CUOZZO GUARNOTTA, USER EXPERIENCE RESEARCH LEAD…`), the author's name, the book title, **and** real sections (`Revision History for the First Edition:`). The Rust binding's flat list contains the same furniture; it simply calls it all level 1.

**Verdict: not material enough to displace the default.** Neither engine turns a 238-page book into ~15 chapters automatically — one flattens, the other nests but mislabels a third of the headings. What the comparison actually settles is a _design_ conclusion, not a backend one:
...
Conversion is lossy in exactly the way that matters: a `bai/source` today keeps the _derived_ markdown and loses the artefact. The vault's own doctrine says the source is the unrecoverable anchor — the thing that cannot be reconstructed once it is gone — so the original bytes belong on the source document, not on a filesystem somewhere.
...
1. ~~Does the PDF path give heading structure with `layout_heron.onnx` alone, or does structure need TableFormer too?~~ **Answered by §6.2:** the Rust binding returns headings but no _hierarchy_ — 97 of 99 chunks at depth 1 on a 20-page sample, 996 of 998 across a 238-page book — and its `ConvertOptions` exposes no level inference at all. So depth is a docling-serve capability (`do_pdf_heading_hierarchy`), not a local-service one.
   **Closed — the default stays the Rust service** (§3). The head-to-head in §11 shows level inference is real but not clean: `docling-serve` nested `Why I Wrote This Book` under `[ Preface ]` correctly, yet put **15 of its 27 headers at level 6** — cover blurbs and the author's name mixed in with real sections. Neither engine derives ~15 chapters from a 238-page book automatically, so the conclusion is a _design_ one: **the section list is user-editable whichever engine runs.** `docling-serve` remains a ~40-line adapter for its other capabilities, or for a future sections UI built to exploit its levels.
