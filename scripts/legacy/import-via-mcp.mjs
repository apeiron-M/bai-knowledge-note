  ...
  for (const line of raw.split("\n")) {
    ...
    if (kv) {
      ...
      if (v === "") {
        ...
        currentList = null;
        fm[currentKey] = v.replace(/^["']/, "").replace(/["']$/, "");
      }
    }
  }
...
for (const file of files) {
  ...
  if (fm.type)
    actions.push({
      type: "SET_NOTE_TYPE",
      input: { noteType: fm.type, updatedAt: created },
      scope: "global",
    });
  if (parsed.content)
    actions.push({
  ...
  for (const [fmKey, schemaField] of Object.entries(STRING_FIELD_MAP)) {
    if (fm[fmKey] && fmKey !== "type") {
      actions.push({
        ...
        input: { field: schemaField, value: fm[fmKey], updatedAt: created },
        scope: "global",
      });
    }
  }
  for (const [fmKey, schemaField] of Object.entries(LIST_FIELD_MAP)) {
    if (fm[fmKey] && Array.isArray(fm[fmKey])) {
      actions.push({
        ...
        input: { field: schemaField, values: fm[fmKey], updatedAt: created },
        scope: "global",
      });
    }
  }
...
// Write batch file
fs.writeFileSync(
  "scripts/mcp-import-batch.json",
  JSON.stringify(batch, null, 2),
);
console.log(
  `Prepared ${batch.length} notes for MCP import → scripts/mcp-import-batch.json`,
);
console.log(
  `Total actions: ${batch.reduce((s, b) => s + b.actions.length, 0)}`,
);
