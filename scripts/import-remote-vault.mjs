...
if (!DRIVE_ID) {
  console.error(
    "Usage: node import-remote-vault.mjs --drive-id <UUID> [--dry-run] [--limit N] [--backup-dir <path>]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  // Sort: root folders first, then children
  const sorted = [];
  const remaining = [...folderStructure];
  while (remaining.length > 0) {
    const batch = remaining.filter(
      (f) => f.parentFolder === null || oldFolderIdToNew.has(f.parentFolder),
    );
    if (batch.length === 0) {
    ...
    sorted.push(...batch);
    for (const b of batch) remaining.splice(remaining.indexOf(b), 1);
  }
  ...
  for (const folder of sorted) {
    ...
    try {
      // Use MCP to add folder to drive
      const actions = [
        {
          type: "ADD_FOLDER",
          input: {
      ...
      const result = await mcpCall("addActions", {
        documentId: DRIVE_ID,
        actions,
      });

      // Try to extract the new folder ID from the result
      // If not available, re-fetch drive to find it
      const driveDoc = await mcpCall("getDocument", { id: DRIVE_ID });
      const nodes = driveDoc?.document?.state?.global?.nodes ?? [];
      const newFolder = nodes.find(
        (n) =>
          n.kind === "folder" &&
          n.name === folder.name &&
          (parentId ? n.parentFolder === parentId : n.parentFolder == null),
      );
  ...
  const docEntries = manifest.documents.filter(
    (d) => d.documentType !== "powerhouse/document-drive",
  );
