      ...
      modules: [
        {
          ...
          operations: [
            ...
            },
            {
              ...
              name: "ADD_ATTACHMENT",
              description:
                "Claim a content-addressed attachment the content refers to (a figure, a formula image, a table, a page, a clip). Idempotent per id.",
              schema:
                "input AddAttachmentInput {\n    id: OID!\n    ref: AttachmentRef!\n    mimeType: String!\n    fileName: String\n    sizeBytes: Int\n    role: String\n    page: Int\n    alt: String\n    width: Int\n    height: Int\n    attachedAt: DateTime!\n}",
              template:
                "Claim a content-addressed attachment the content refers to (a figure, a formula image, a table, a page, a clip). Idempotent per id.",
              reducer:
                "if (state.attachments.some((attachment) => attachment.id === action.input.id)) {\n  throw new DuplicateAttachmentIdError(`Attachment ${action.input.id} is already on this source`);\n}\nstate.attachments.push({\n  id: action.input.id,\n  ref: action.input.ref,\n  mimeType: action.input.mimeType,\n  fileName: action.input.fileName || null,\n  sizeBytes: action.input.sizeBytes ?? null,\n  role: action.input.role || null,\n  page: action.input.page ?? null,\n  alt: action.input.alt || null,\n  width: action.input.width ?? null,\n  height: action.input.height ?? null,\n  attachedAt: action.input.attachedAt,\n});",
              errors: [
                {
                  id: "err-duplicate-attachment-id",
                  name: "DuplicateAttachmentIdError",
                  code: "DUPLICATE_ATTACHMENT_ID",
                  description:
                    "An attachment with this id is already on the source",
                  template: "",
                },
              ],
              examples: [],
            ...
            },
            {
              ...
              description:
                "Drop an attachment from the source; the bytes stay in the store until nothing references them.",
              schema: "input RemoveAttachmentInput {\n    id: OID!\n}",
              template:
                "Drop an attachment from the source; the bytes stay in the store until nothing references them.",
              reducer:
                "const index = state.attachments.findIndex((attachment) => attachment.id === action.input.id);\nif (index === -1) {\n  throw new AttachmentNotFoundError(`No attachment ${action.input.id} on this source`);\n}\nstate.attachments.splice(index, 1);",
              errors: [
                {
                  id: "err-attachment-not-found",
                  name: "AttachmentNotFoundError",
                  code: "ATTACHMENT_NOT_FOUND",
                  description: "No attachment with this id is on the source",
                  template: "",
                },
              ],
              examples: [],
