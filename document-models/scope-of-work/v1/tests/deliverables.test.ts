...
import {
  ...
  linkDeliverableGoal,
  LinkDeliverableGoalInputSchema,
  reducer,
  removeDeliverable,
...
describe("DeliverablesOperations", () => {
  ...
  it("should handle linkDeliverableGoal operation", () => {
    const document = utils.createDocument();
    const input = generateMock(LinkDeliverableGoalInputSchema());

    const updatedDocument = reducer(document, linkDeliverableGoal(input));

    expect(isScopeOfWorkDocument(updatedDocument)).toBe(true);
    expect(updatedDocument.operations.global).toHaveLength(1);
    expect(updatedDocument.operations.global[0].action.type).toBe(
      "LINK_DELIVERABLE_GOAL",
    );
    expect(updatedDocument.operations.global[0].action.input).toStrictEqual(
      input,
    );
